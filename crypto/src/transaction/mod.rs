...
impl Transaction {
    ...
    pub fn issue(
        ...
        trx.issuer_sig = issuer.sign(&trx);
    ...
    pub fn approve(&mut self, receiver: &dyn Signer) {
        self.receiver_sig = receiver.sign(self);
    }

    /// Validate transaction against issuer signature.
    pub fn validate_for_issuer(&self, validator: &dyn Verifier) -> bool {
        if let Err(_) = validator.validate_other(self, &self.issuer_sig, self.issuer.as_ref()) {
            return false;
        }
    ...
    pub fn validate_for_receiver(&self, validator: &dyn Verifier) -> bool {
        if let Err(_) = validator.validate_other(self, &self.receiver_sig, self.receiver.as_ref()) {
            return false;
        }
...
impl AsVecBytes for Transaction {
    #[inline]
    fn as_vec_bytes(&self) -> Vec<u8> {
        let mut buffer: Vec<u8> = Vec::with_capacity(self.estimated_size());

        buffer.extend_from_slice(self.subject.as_bytes());
        buffer.extend_from_slice(&self.data);
        buffer.extend_from_slice(&self.created_at.to_ne_bytes());
        buffer.extend_from_slice(self.issuer.as_bytes());
        buffer.extend_from_slice(self.receiver.as_bytes());
...
mod tests {
    ...
    impl Signer for SignerMock {
        fn sign(&self, _: &dyn AsVecBytes) -> Vec<u8> {
    ...
    fn issue() {
        ...
        for _ in 0..rounds {
            ...
            assert_eq!(trx.as_vec_bytes().len(), trx.estimated_size());
        }
    }
